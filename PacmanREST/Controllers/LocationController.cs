...
namespace PacmanREST.Controllers
{
    public class LocationController : ApiController
    {
        ...
        public IHttpActionResult PostPacman_location_db(Pacman_location_db pacman_location_db)
        {
            ...
            db.SaveChanges();
            if ((pacman_location_db.id_patient != null) && (pacman_location_db == null))
            {
                checkFence cf = new checkFence();
                cf.patientx = pacman_location_db.coordinates_x;
                cf.patienty = pacman_location_db.coordinates_y; 
                var fence = (from f in db.Pacman_fence_db
                             where (f.id_patient == pacman_location_db.ID) && (f.id_carer > 0)
